...
class DBUtils():
    ...
    def execute(self,sql,param=None):
        try:
            if self.__conn and self.__cursor:
                if param:
                    res = self.__cursor.execute(sql,param)
    ...
    def insert(self,sql,param=None):#insert语句  插入数据
        ...
        print(param)
        res = self.execute(sql,param)
        #logger.info("成功执行:"+sql+'  ')
        print(res)
        self.commit()
        self.close()
        return res
    def selectallInfo(self, sql, param=None):
    ...
    def selectoneInfo(self,sql,param=None):

        self.getConnection()
        res = self.executeQuery(sql, param, row='one')
        self.close()
        return res
